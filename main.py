...
# @app.get("/pandas_op/")
def pandas_op(file_name: str, pandas_op: str):
	pd_file = read_file(file_name)
	return pandas_abstract(pd_file, pandas_op)

def pandas_abstract(pd_file, pandas_op):
	pd_func = pd_mapper[pandas_op]
...
# @app.get("/fit_loaded/")
def fit_loaded(id: int, model_name: str, model_params: Optional[str] = ""):
	if (len(loaded_files) <= id):
		return {"error" : "wrong id"}
	return fit_model(loaded_files[id], model_name, model_params)

def pandas_loaded(id: int, pandas_op: str):
	return pandas_abstract(loaded_files[id], pandas_op)
